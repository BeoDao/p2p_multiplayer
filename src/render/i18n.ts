...
const STRINGS: Record<Lang, Record<string, string>> = {
  en: {
    ...
    help2: 'LMB: fire/dig/build (hold) · RMB: breach shield (rifleman) / scope (marksman) / detonate C4 (engineer) · S: crouch · {1}~{9}/wheel: item · {F1}~{F3} change class in base',
    help3: '{E} buy at supply depot (iron) · heal at depot · {Tab} scoreboard · {Enter} chat · {Esc} settings',
    ...
    phase_discover: 'Looking for session', phase_joining: 'Joining', phase_playing: 'Playing', phase_resync: 'Resyncing', phase_full: 'Room full',
    ...
    dead: 'Dead', respawnIn: 'Respawn in {s}s', baseHint: '{F1} {c0} · {F2} {c1} · {F3} {c2}',
    ...
    loadReplay: 'Load replay (.rwr)', replaySaved: 'Replay saved',
    reconnecting: "Connection failed — reconnecting (#{n})",
    roomFull: 'Room is full ({n} players max)', roomFullHint: 'Try another room code, or wait for someone to leave and reload.',
    mountHint: '{E} Board the {v}', dismountHint: '{A}{D} drive · {E} get off', gunnerHint: 'LMB fire · {E} get off',
  },
  ja: {
    ...
    help2: '左クリック: 射撃/採掘/建築（長押し） · 右クリック: 突入シールド（歩兵）/ 照準（狙撃手）/ C4起爆（工兵） · S: しゃがみ · {1}~{9}/ホイール: アイテム · 拠点で {F1}~{F3} クラス変更',
    help3: '補給所で {E} 購入（鉄） · 補給所で回復 · {Tab} スコア · {Enter} チャット · {Esc} 設定',
    ...
    phase_discover: 'セッションを検索中', phase_joining: '参加中', phase_playing: 'プレイ中', phase_resync: '再同期中', phase_full: '満員',
    ...
    dead: '死亡', respawnIn: '{s}秒後に復活', baseHint: '{F1} {c0} · {F2} {c1} · {F3} {c2}',
    ...
    loadReplay: 'リプレイを読み込む (.rwr)', replaySaved: 'リプレイを保存しました',
    reconnecting: "接続に失敗 — 再接続中 (#{n})",
    roomFull: 'ルームが満員です（最大{n}人）', roomFullHint: '別のルームコードを試すか、誰かが退出してから再読み込みしてください。',
    mountHint: '{E} {v}に乗る', dismountHint: '{A}{D} 運転 · {E} 降りる', gunnerHint: '左クリック 射撃 · {E} 降りる',
  },
  ru: {
    ...
    help2: 'ЛКМ: огонь/копать/строить (удерж.) · ПКМ: штурмовой щит (стрелок) / прицел (снайпер) / подрыв C4 (сапёр) · S: присесть · {1}~{9}/колесо: предмет · {F1}~{F3} смена класса на базе',
    help3: '{E} купить на складе (железо) · лечение на складе · {Tab} таблица · {Enter} чат · {Esc} настройки',
    ...
    phase_discover: 'Поиск сессии', phase_joining: 'Подключение', phase_playing: 'Игра', phase_resync: 'Ресинхронизация', phase_full: 'Заполнено',
    ...
    dead: 'Погиб', respawnIn: 'Возрождение через {s} с', baseHint: '{F1} {c0} · {F2} {c1} · {F3} {c2}',
    ...
    loadReplay: 'Открыть повтор (.rwr)', replaySaved: 'Повтор сохранён',
    reconnecting: "Сбой соединения — переподключение (#{n})",
    roomFull: 'Комната заполнена (максимум {n} игроков)', roomFullHint: 'Попробуйте другой код комнаты или дождитесь, пока кто-то выйдет, и обновите страницу.',
    mountHint: '{E} Сесть: {v}', dismountHint: '{A}{D} ехать · {E} слезть', gunnerHint: 'ЛКМ огонь · {E} слезть',
  },
  zh: {
    ...
    help2: '左键：射击/挖掘/建造（按住） · 右键：突击盾（步枪兵）/ 瞄准（狙击手）/ 引爆C4（工兵） · S：蹲下 · {1}~{9}/滚轮：物品 · 基地内 {F1}~{F3} 切换职业',
    help3: '补给站上 {E} 购买（铁） · 补给站上回血 · {Tab} 记分板 · {Enter} 聊天 · {Esc} 设置',
    help4: '把敌方旗帜带回己方旗帜处得分（3 分获胜）',
    chatPlaceholder: '输入消息，Enter 发送（Esc 取消）',
    settings: '设置', volume: '音效音量', zoom: '画面缩放', closeHint: '按 Esc 关闭',
    phase_discover: '正在查找会话', phase_joining: '加入中', phase_playing: '游戏中', phase_resync: '重新同步中', phase_full: '已满',
    ...
    dead: '阵亡', respawnIn: '{s} 秒后复活', baseHint: '{F1} {c0} · {F2} {c1} · {F3} {c2}',
    ...
    loadReplay: '加载回放 (.rwr)', replaySaved: '回放已保存',
    reconnecting: "连接失败 — 正在重连 (#{n})",
    roomFull: '房间已满（最多 {n} 人）', roomFullHint: '请尝试其他房间代码，或等有人离开后刷新。',
    mountHint: '{E} 乘坐{v}', dismountHint: '{A}{D} 驾驶 · {E} 下车', gunnerHint: '左键 射击 · {E} 下车',
  },
  ar: {
    ...
    help2: 'الزر الأيسر: إطلاق/حفر/بناء (استمرار) · الأيمن: درع اقتحام (الرامي) / تصويب (القناص) / تفجير C4 (المهندس) · S: انبطاح · {1}~{9}/العجلة: الأداة · {F1}~{F3} تغيير الفئة في القاعدة',
    help3: '{E} شراء في مستودع الإمداد (حديد) · الشفاء في المستودع · {Tab} لوحة النتائج · {Enter} دردشة · {Esc} الإعدادات',
    ...
    phase_discover: 'البحث عن جلسة', phase_joining: 'جارٍ الانضمام', phase_playing: 'جارٍ اللعب', phase_resync: 'إعادة المزامنة', phase_full: 'ممتلئة',
    ...
    dead: 'مات', respawnIn: 'العودة بعد {s} ث', baseHint: '{F1} {c0} · {F2} {c1} · {F3} {c2}',
    ...
    loadReplay: 'تحميل إعادة (.rwr)', replaySaved: 'تم حفظ الإعادة',
    reconnecting: "فشل الاتصال — إعادة الاتصال (#{n})",
    roomFull: 'الغرفة ممتلئة (الحد الأقصى {n} لاعبين)', roomFullHint: 'جرّب رمز غرفة آخر، أو انتظر مغادرة أحدهم ثم أعد التحميل.',
    mountHint: '{E} اركب {v}', dismountHint: '{A}{D} قيادة · {E} نزول', gunnerHint: 'الزر الأيسر إطلاق · {E} نزول',
  },
};
...
export function detectLang(): Lang {
  ...
  try { saved = localStorage.getItem('rw.lang'); } catch { /* ignore */ }
...
export function setLang(l: Lang): void {
  ...
  try { localStorage.setItem('rw.lang', l); } catch { /* ignore */ }
}
